...
export default function ScatterPlot() {
	...
	async function drawChart() {
		...
		const Bound = Wrapper.append("g").style(
			"transform",
			`translate(${wrapper.padding}px, ${wrapper.padding}px)`,
		);

		// 2. Mangle Data
		const rawData = await d3.tsv(
		...
		Bound.selectAll("circle")
			...
			.join("circle")
			.filter(
				(d) => !(Number.isNaN(xAccessor(d)) || Number.isNaN(yAccessor(d))),
			)
  ...
  `;
	...
	return <div id={id} style={{ width: "100%", height: "600px" }} />;
}
